...
void draw_gate(const Cell *n1, const Cell *n2) // Affiche une "porte" qui symbolise un passage entre 2 celules reliez.
{
...
void display_cell(Cell *cell) // Permet d'afficher le labyrinthe.
{
  vibes::drawBox(cell->m_x, cell->m_x + 1, cell->m_y, cell->m_y + 1, "[lightGray]"); // Affiche la première cellule.
  cell->m_displayed = true;

  for(int i = 0 ; i < cell->m_nb_neighb ; i++) // Affiche les autres cellules de façon récursive.
  {
    if(!cell->m_neighb[i]->m_displayed)
      display_cell(cell->m_neighb[i]);
    draw_gate(cell, cell->m_neighb[i]); // Affiche le passage entre 2 cellules reliez.
  }
}
...
void save_maze(const Maze& maze, const string& file_name)
{
  ...
  if(!ofst.is_open()) cout << "Erreur d’ouverture de " << file_name << endl;
